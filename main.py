...
def load_vgg(sess, vgg_path):
    ...
    layer_in = graph.get_tensor_by_name(vgg_input_tensor_name)
    ...
    return layer_in, keep_prob, layer_3, layer_4, layer_7
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    shaped_logits = tf.reshape(nn_last_layer, (-1, num_classes))
    shaped_labels = tf.reshape(correct_label, (-1, num_classes))
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=shaped_logits, labels=shaped_labels))
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cross_entropy_loss)
    return shaped_logits, optimizer, cross_entropy_loss
...
def run():
    ...
    with tf.Session() as sess:
        ...
        labels = tf.placeholder(tf.int32, shape=(None, None, None, num_classes), name='label')
        learning_rate = tf.placeholder(tf.float32, name='learn_rate')
        logits, opt, loss = optimize(layer_output, labels, learning_rate, num_classes)
        print("Model assembled... starting run...")
        sess.run(tf.global_variables_initializer())
        
        train_nn(sess, epochs, batch_size, get_batches_fn, opt, loss, input_image, labels, keep_prob, learning_rate)
